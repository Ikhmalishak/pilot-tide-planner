...
function ImportSection() {
  ...
  const handleImport = async () => {
    ...
    setMsg(null);
    try {
      const result = await uploadApi.tideIndicators(
        file,
      ...
      let text = `Imported ${result.inserted} indicators. Navigation generated for ${genOk} dates.`;
      if (genFail.length > 0) {
        text += ` Failed: ${genFail.map((f: any) => `${f.date} (${f.error})`).join(', ')}`;
      }
      setMsg({ type: genFail.length > 0 && genOk === 0 ? 'error' : 'success', text });
      queryClient.invalidateQueries({ queryKey: ['tide-indicators'] });
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
      <h3 className="font-semibold text-gray-900 mb-4">Import Monthly Tide File</h3>
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          >
            {file ? (
              ...
              <p className="text-sm text-gray-500">Select ttables*_mthly.xls</p>
            )}
          </div>
